 ...
 * 
 * http// // Match only these URLs .requestMatchers()//
 * .antMatchers("/api/**")// .antMatchers("/ui/**")//
 * .and().authorizeRequests()//
 * .antMatchers("/api/admin/**").hasAuthority("admin")//
 * .antMatchers("/api/**").hasAnyAuthority("admin", "standard")//
 * .antMatchers("/ui/admin/**").hasAuthority("admin")//
 * .antMatchers("/ui/**").hasAnyAuthority("admin", "standard")// // Ignore CSRF
 * and CORS .and().csrf().disable().cors().disable();
